 ...
 * For hackathon / offline: use chatWithOllama() below which calls the local
 * FastAPI /api/chat endpoint (powered by Ollama qwen2.5:0.5b on your laptop).
 */
...
// ─── Ollama / Local LLM (Hackathon offline stack) ─────────────────────────────
/**
 * Chat via local Ollama qwen2.5:0.5b through FastAPI /api/chat.
 * Uses shared api.js chatAI (no duplicate BASE_URL).
 *
 * @param {string} systemPrompt  - Ignored (context set via opts.context)
 * @param {string} userMessage   - User query (any language)
 * @param {object} opts          - { lang='en', context='loan', crop, acres }
 * @returns {Promise<string>}    - AI response text
 */
export async function chatWithOllama(systemPrompt, userMessage, opts = {}) {
  const { lang = 'en', context = 'loan', crop = null, acres = null } = opts
  const res = await chatAI(userMessage, lang, context, crop, acres)
  return res.data?.response || ''
}

/**
 * Smart chat: tries Ollama backend first, falls back to OpenAI GPT.
 * Recommended replacement for chatWithGPT() across all screens.
 *
 * @param {string} systemPrompt  - OpenAI-style system prompt (used in GPT fallback)
 * @param {string} userMessage   - User query
 * @param {object} opts          - Options passed to chatWithOllama
 */
export async function chatSmart(systemPrompt, userMessage, opts = {}) {
  try {
    return await chatWithOllama(systemPrompt, userMessage, opts)
  } catch (ollamaErr) {
    console.warn('[chatSmart] Ollama unavailable, trying OpenAI:', ollamaErr.message)
    try {
      return await chatWithGPT(systemPrompt, userMessage)
    } catch (gptErr) {
      throw new Error(`All AI backends offline. Ollama: ${ollamaErr.message}`)
    }
  }
}
