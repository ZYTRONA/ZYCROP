...
import tensorflow as _tf_raw  # type: ignore[import-untyped]
tf: Any = _tf_raw  # cast Unknown → Any so attribute access returns Any
from tensorflow.keras import layers as _layers_raw, Model as _Model_raw  # type: ignore[import-untyped]
layers: Any = _layers_raw
Model: Any = _Model_raw
from tensorflow.keras.applications import MobileNetV2 as _MV2_raw  # type: ignore[import-untyped]
MobileNetV2: Any = cast(Any, _MV2_raw)
from tensorflow.keras.callbacks import EarlyStopping as _ES_raw, ReduceLROnPlateau as _RLROP_raw  # type: ignore[import-untyped]
EarlyStopping: Any = _ES_raw
ReduceLROnPlateau: Any = _RLROP_raw
...
try:
    import tensorflow_datasets as _tfds_raw  # type: ignore[import-untyped]
except ImportError:
    raise SystemExit(
...
tfds: Any = _tfds_raw  # cast Unknown → Any
ds_train_raw: Any
ds_val_raw: Any
ds_info: Any
...
NUM_CLASSES: int = ds_info.features['label'].num_classes  # type: ignore[attr-defined]
CLASS_NAMES: list[str] = list(ds_info.features['label'].names)  # type: ignore[attr-defined]
print(f'      Loaded {NUM_CLASSES} disease classes.')

# ─── Preprocessing ────────────────────────────────────────────────────────────
def preprocess(image: Any, label: Any) -> tuple[Any, Any]:
    image = tf.image.resize(image, [IMG_SIZE, IMG_SIZE])
    image = tf.cast(image, tf.float32) / 255.0
    return image, label

def augment(image: Any, label: Any) -> tuple[Any, Any]:
...
x: Any     = base_model(inputs, training=False)
